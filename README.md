...
- **filesystem_generate_pe_wannacry**: Generate PE files with WannaCry malware signatures (Layer 2)
- **filesystem_execute_ransomware_profile**: Execute complete ransomware simulation (Layer 3)
...
- **network_generate_syn_flood**: Generates SYN Flood attack traffic (Layer 3)
- **network_generate_network_user_profile**: Generates user behavior profiles (Layer 3)
- **network_generate_wannacry_attack**: Generates WannaCry-style multi-stage attack simulation (Layer 3)

Example of using a module:

```yaml
- name: Create office worker profile
  tectonic.evidence_generation.generate_fs_user_profile:
